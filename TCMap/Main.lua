...
ChatReceived = function(sender, args)
    ...
    if message~=nil then
        local t = extractPatterns(message, "%a-%s?%a*TC:", "(%d%d?%.%d%u%s?%d%d?%.%d%u)",-3); -- extract map and coordinates
        if t~=nil then
            for i=1,mainwindow.nrMaps do
                if mainwindow.maps[i].location==t[1] then mainwindow.maps[i]:AddMarked( t ); end
            end
        end
    end
end
